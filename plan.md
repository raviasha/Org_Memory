...
5. - [x] Session 5, Data Track: git connector MVP.
...
5b. - [x] Session 5b, Data Track: document upload, image ingest, and folder upload pipeline.
...
- Delete project: trigger the delete action on an existing project; the project is removed from the active list and a soft-delete audit entry is visible.
- Add file to project: select an existing project and upload a file through the asset-add UI; the asset appears in the project workspace table with status "pending" or "processing".
...
- Subtask curation bundle (Step 7): `POST /v1/subtasks/{subtask_id}/curate` returns a bundle that includes: selected memory item IDs with inclusion reasons, dropped memory item IDs with exclusion reasons, selected asset IDs, a rationale trace per selected item, an immutable snapshot_id, and a token budget usage summary; the bundle must not contain duplicate evidence items across stores.
...
Exit criteria: upload an asset through Session 5b pipeline (or stub path); canonical asset memory exists at deterministic path in the project store; idempotent re-upload updates rather than duplicates; ingest is blocked when memory write fails; retry exhaustion sets `ingest_status = blocked_on_memory_write` and asset record remains intact with no data loss; SCHEMA.md injection is implemented and version check passes; Playwright API tests cover create-store, seed, idempotent update, failure-gate, retry-exhaustion paths, and SCHEMA.md version validation. Additionally, the following integration tests must pass — covering requirement Steps 2, 3, and 5 end-to-end:
- Ingest pipeline sequence (Step 2): a Playwright API test uploads a file, asserts the asset row is stored in Supabase assets, then confirms the Managed Agents memory API was called and a canonical asset memory file exists at the deterministic path `/assets/{asset_id}.md` in the project memory store; the memory record must contain the required provenance fields (asset_id, project_id, source_uri, acl_scope, ingest_run_id, memory_version_id).
...
Exit criteria: automated scorecard generated from benchmark suite. Specifically, the following tests must pass — covering requirement Step 9 (cross-store reasoning and eval execution):
- Eval runner execution: a test runner script (or Playwright API test suite) loads all 10 eval case files from `eval/cases/` and executes each against the real inference pipeline via `POST /v1/tasks/curate` and `POST /v1/subtasks/{subtask_id}/curate`; no case may be skipped or stubbed.
- Per-case recall and precision: for each eval case, the runner computes Recall@5 and Precision@5 against `required_asset_ids` and `relevant_asset_ids`; required-asset hit rate (all `required_asset_ids` present in top-5) must be reported per case.
- nDCG@5 reporting: nDCG@5 is computed across all cases and written to a scorecard file; the benchmark gate fails if the aggregate nDCG@5 falls below the acceptance threshold in the Acceptance Thresholds section.
- Cross-store reasoning (Step 9): at least one eval case (task-009-vendor-compliance or task-001-npv-recommendation) must involve stores from more than one project; the eval runner must assert that the curated bundle selected artifacts from the correct stores (based on `relevant_asset_ids`) rather than over-fetching from a single store; the curation manifest for that case must show that the agent's store selection covered the required cross-project artifacts without loading all stores.
- Scorecard persistence: the scorecard output is written to a versioned file in `eval/results/` and the benchmark gate result (pass/fail per threshold) is printed to stdout; CI must fail if any required-asset hit rate falls to zero for any eval case.
